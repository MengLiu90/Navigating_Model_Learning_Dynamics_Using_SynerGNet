...
class Net(torch.nn.Module):
    def __init__(self, num_layers, num_node_features, num_classes):
        ...
        for i in range(num_layers):
            if i == 0:
                self.conv_layers.append(TransformerConv(in_channels=num_node_features, out_channels=512,
                                                        heads=1, concat=True, beta=False, dropout=0))
            else:
                self.conv_layers.append(TransformerConv(in_channels=512, out_channels=512, heads=1,
                                                        concat=True, beta=False, dropout=0))
    ...
    def reset_parameters(self):
        for module in self.modules():
            if isinstance(module, (Linear, TransformerConv, BatchNorm1d)):
