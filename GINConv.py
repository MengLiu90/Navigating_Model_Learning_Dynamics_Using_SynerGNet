...
class Net(torch.nn.Module):
    def __init__(self, num_layers, num_node_features, num_classes):
        ...
        for i in range(num_layers):
            if i == 0:
                self.conv_layers.append(GINConv(Linear(num_node_features, 512), eps=1, train_eps=True))
            else:
                self.conv_layers.append(GINConv(Linear(512, 512), eps=0, train_eps=True))
    ...
    def reset_parameters(self):
        for module in self.modules():
            if isinstance(module, (Linear, GINConv, BatchNorm1d)):
