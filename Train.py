...
def Train(synergy_file_path, h5py_dir_path, module_name):
    df = pd.read_csv(synergy_file_path)
    # df = pd.read_csv('./Example_data/drugcombs_synergy_data.csv')
    ...
    # save the model with the best bac
...
if __name__ == "__main__":
    ...
    parser.add_argument("module_name", help='Model name')

    # Parse the command-line arguments
    args = parser.parse_args()

    # Call the process_data function with the provided file paths
    Train(args.csv_file, args.h5py_dir, args.module_name)
