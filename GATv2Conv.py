...
class Net(torch.nn.Module):
    def __init__(self, num_layers, num_node_features, num_classes):
        ...
        for i in range(num_layers):
            if i == 0:
                self.conv_layers.append(GATv2Conv(num_node_features, 512, heads=1, concat=False))
            else:
                self.conv_layers.append(GATv2Conv(512, 512, heads=1, concat=False))
    ...
    def reset_parameters(self):
        for module in self.modules():
            if isinstance(module, (Linear, GATv2Conv, BatchNorm1d)):
